...
Если пользователь уже виден в Supabase Auth → Users, но сайт пишет `Invalid login credentials`, выполнить `supabase/confirm-admin-user.sql`. Этот SQL подтверждает email в Auth и гарантирует профиль `ADMIN`.

Для простого форумного сценария можно не использовать Supabase Auth для админа: выполнить `supabase/quick-admin-promos.sql`, открыть `/quick-admin`, ввести код `DNF-ADMIN-2026` и генерировать промокоды.
