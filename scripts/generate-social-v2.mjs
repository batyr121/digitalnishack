...
const partnerSlides = [
  ['01', 'General Partner', 'Artisan Education поддерживает форум как главный партнёр: идеи, команды и цифровое будущее.', '◆', artisanLogo],
...
const slides = [
  ...
  frame('partners-01', 'partners', 1, {
    className: 'xl',
    html: `<img class="brand" src="${logo}" alt=""><div class="topline">19.09.2026<br>ПАРТНЁРЫ</div><div class="kicker"><b>11</b>Поддержка форума</div><h1>Вместе<br>делаем форум.</h1><p>Партнёры помогают собрать людей, технологии и идеи в одном пространстве.</p><div class="partner-stack">${partnerSlides.map(([n, title, desc, icon, partnerLogo]) => `<article class="partner-row">${partnerLogo ? `<img class="partner-logo-img" src="${partnerLogo}" alt="Artisan Education">` : `<div class="mark">${icon}</div>`}<div><h2>${title}</h2>${partnerLogo ? '<span class="partner-name">Artisan Education</span>' : ''}<p>${desc}</p></div></article>`).join('')}</div><div class="footer"><span>General · Technical · Operational</span><span>01 / 04</span></div>`,
  }),
  ...partnerSlides.map(([n, title, desc, icon, partnerLogo], i) =>
    frame(`partners-${String(i + 2).padStart(2, '0')}`, 'partners', i + 2, {
      className: 'xl',
      html: `<img class="brand" src="${logo}" alt=""><div class="topline">ПАРТНЁРЫ<br>${n}</div><div class="kicker"><b>${n}</b>DIGITAL NIS FORUM</div><h1>${title}</h1><article class="partner-frame">${partnerLogo ? `<img class="partner-logo-img" src="${partnerLogo}" alt="Artisan Education">` : `<div class="mark">${icon}</div>`}<div><h2>${partnerLogo ? 'Artisan Education' : title}</h2><p>${desc}</p></div><span class="num">${n}</span></article><div class="footer"><span>Спасибо за поддержку</span><span>${String(i + 2).padStart(2, '0')} / 04</span></div>`,
    }),
  ),
