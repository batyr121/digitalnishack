...
const slides = [
  ...
  frame('competitions-01', 'competitions', 1, {
    className: 'xl',
    html: `<img class="brand" src="${logo}" alt=""><div class="topline">19.09.2026<br>СОРЕВНОВАНИЯ</div><div class="kicker"><b>06</b>Для тех, кто создаёт</div><h1>Твоя идея.<br>Твоя игра.<br>Твой ход.</h1><div class="cards">${competitions.map((c, i) => `<article class="card ${i === 3 ? 'active' : ''}"><div><span class="icon">${c[3]}</span><h2 class="${c[1].length > 28 ? 'long' : ''}">${c[1]}</h2><p>${c[2]}</p></div><div class="footer" style="position:static;padding-top:18px"><span>19 сентября</span><span>↗</span></div></article>`).join('')}</div><div class="footer"><span>4 направления</span><span>01 / 05</span></div>`,
  }),
